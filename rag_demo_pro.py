...
SEARCH_ENGINE = "google"  # 如有需要，也可以换成其他搜索引擎

# 新增功能：配置 rerank 的方法（使用 CrossEncoder 或 LLM）
RERANK_METHOD = os.getenv("RERANK_METHOD", "cross_encoder")  # 可选 "cross_encoder" 或 "llm"

# 新增功能：SiliconFlow API 配置
SILICONFLOW_API_KEY = os.getenv("SILICONFLOW_API_KEY")
SILICONFLOW_API_URL = os.getenv("SILICONFLOW_API_URL", "https://api.siliconflow.cn/v1/chat/completions")

# 设置 requests 网络请求的全局重试次数（防止短暂失败）
import requests

requests.adapters.DEFAULT_RETRIES = 3  # 增加请求重试次数为 3

# 禁用 oneDNN 优化（TensorFlow 在某些环境下数值误差会增大）
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'  # 关闭 oneDNN 优化（确保结果稳定）

# 设置不走代理的地址（本地通信时避免被代理打断）
os.environ['NO_PROXY'] = 'localhost,127.0.0.1'  # 设置本地地址不使用代理
...
# 初始化 FAISS 相关的全局变量
faiss_index = None  # FAISS 索引对象
faiss_contents_map = {}  # original_id -> 文本内容（索引对应的原文）
faiss_metadatas_map = {}  # original_id -> 元信息（如页码、文件名等）
faiss_id_order_for_index = []  # 按添加顺序记录所有文档块 ID，用于 FAISS 中索引映射

# 新增功能：初始化 CrossEncoder（延迟加载，线程安全）
cross_encoder = None
cross_encoder_lock = threading.Lock()  # 用于确保多线程环境中只加载一次 CrossEncoder
